...
fn read_fvec() -> Vec<f32> {
    ...
    println!("Enter a list of float numbers");
    for line in stdin.lock().lines() {
        let line = line.unwrap();
        
        match line.trim().parse::<f32>() {
            Ok(fnum) => vec.push(fnum),
            Err(_) => {
                println!("is not a float number")
            },
        }
    }
...
pub fn main() {
    let vec = read_t_vec::<i32>();
    let min = vec_min(vec);
    min.print2();
    let fvec = read_t_vec::<f32>();
